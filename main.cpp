...
int main(int argc, char *argv[])
{
    ...
    ifstream fileHighScore(HIGH_SCORE_FILE);
    ...
    {
        fileHighScore >> highScore >> namePlayer_highScore;
    }
    namePlayer_highScore = "High Score: " + namePlayer_highScore;
    pair<int, string> rank_pair = getRank(rankSprite, highScore);
    string rank_name = to_string(highScore) + " " + rank_pair.second;

    SDL_Texture* highRankScoreTexture = graphics.renderText(rank_name.c_str(), size_50, orange);
    SDL_Texture* highRankScoreNamePlayerTexture = graphics.renderText(namePlayer_highScore.c_str(), size_50, white);
    rankSprite.currentFrame = rank_pair.first;
...
//    graphics.presentScene();
    ...
    while(!quit_menu)
    {
        ...
        if(!readGuide)
        {
            graphics.renderTexture(highRankScoreNamePlayerTexture, 70, 300);
            graphics.renderTexture(highRankScoreTexture, 120, 500);
            graphics.renderSprite(170, 300, rankSprite);
        }
